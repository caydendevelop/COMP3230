...
int* getSplitIndices(int low, int high)
{
	...
	// Get 5 indices: low, mid1, mid2, mid3, high
	int* pMidIndices = (int*) malloc(sizeof(int)*5);
	for(int i = 0; i<5; i++){
		if(i==4){ 
			pMidIndices[i] = high;
		}
...
{
	//int count_exit = 0;
	//int* pCount = &count_exit;

	int size_array = high - low;
	int* pMidIndices = getSplitIndices(low, high); // 5 indices: low, mid1, mid2, mid3, high
	...
	for(int i=0;i<3;i++)
	{
		...
		if(fpid_1 == 0){
			...
			mergesort_4_way_rec(array, pMidIndices[i] ,pMidIndices[i+1]);
			printf("Process ID: %d; Sorted integers: ", getpid());
			printArray(array,  pMidIndices[i] ,pMidIndices[i+1]);
			exit(0);
		}
	}

	// Parent process
	mergesort_4_way_rec(array,pMidIndices[3], pMidIndices[4]);
	printf("Process ID: %d; Sorted integers: ", getpid());
	printArray(array, pMidIndices[3], pMidIndices[4]);
	...
	merge_4_way(array, pMidIndices[0], pMidIndices[1],  pMidIndices[2], pMidIndices[3], pMidIndices[4]);
	printf("Process ID: %d; Merged %d integers: ", getpid(), high - low);
	printArray(array, pMidIndices[0], pMidIndices[4]);
}

void recursiveMergesort(int* array, int low, int high, int max_num)
{
	int* pMidIndices = getSplitIndices(low, high);  //5 indices: low, mid1, mid2, mid3, high
	//printArray(pMidIndices, 0, 5);
	...
	for(int i=0;i<3;i++){
		...
		pids[i] = fpid_1;
		if(fpid_1 == 0){
			// The ith child process
			if(pMidIndices[i+1] - pMidIndices[i] > max_num){
				recursiveMergesort(array, pMidIndices[i] ,pMidIndices[i+1], max_num);
			}
			else{
				mergesort_4_way_rec(array, pMidIndices[i] ,pMidIndices[i+1]);
				printf("Process ID: %d; Sorted %d integers: ", getpid(), pMidIndices[i+1] - pMidIndices[i]);
				printArray(array, pMidIndices[i] ,pMidIndices[i+1]);
			}
			exit(0);
		}
	...
	// Parent process
	{
		if(pMidIndices[4] - pMidIndices[3] > max_num){
			//printf("debug...\n\n");
			recursiveMergesort(array, pMidIndices[3], pMidIndices[4], max_num);
			for(int i=0;i<3;i++){
				waitpid(pids[i],  &stat, 0);
			}
